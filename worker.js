...
// @ts-ignore
// https://github.com/Kurakavaaa/Kurakavaaa.git

import { connect } from 'cloudflare:sockets';

// How to generate your own UUID:
// https://www.uuidgenerator.net/
let userID = 'e059edf8-32e7-450e-b95d-b949ca33d516';

// https://www.nslookup.io/domains/Ralyand.yousef.isegaro.com/dns-records/
const proxyIPs= ['Ralyand.yousef.isegaro.com'];
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (typeof env.Ralyand !== 'object') {
                            const errorPage = renderErrorPage('KV Dataset is not properly set!', null, true);
                            return new Response(errorPage, { status: 200, headers: {'Content-Type': 'text/html'}});
                        }

                        const pwd = await env.Ralyand.get('pwd');
                        ...
                        const proxySettings = await env.Ralyand.get('proxySettings', {type: 'json'});
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.Ralyand.get('secretKey');
                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.Ralyand.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.Ralyand.get('pwd');
                    ...
                    case '/panel/password':
                        ...
                        const oldPwd = await env.Ralyand.get('pwd');
                        ...
                        await env.Ralyand.put('pwd', newPwd);
...
const getNormalConfigs = async (env, hostName, client) => {
    ...
    try {
        proxySettings = await env.Ralyand.get("proxySettings", {type: 'json'});
    } catch (error) {
        console.log(error);
...
const generateRemark = (index, port) => {
    ...
    switch (index) {
        case 0:
        case 1:
            remark = `😶‍🌫️ Ralyand - Domain_${index + 1} : ${port}`;
            break;
        case 2:
        case 3:
            remark = `😶‍🌫️ Ralyand - IPv4_${index - 1} : ${port}`;
            break;
        case 4:
        case 5:
            remark = `😶‍🌫️ Ralyand - IPv6_${index - 3} : ${port}`;
            break;
        default:
            remark = `😶‍🌫️ Ralyand - Clean IP_${index - 5} : ${port}`;
            break;
    }
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '😶‍🌫️ Ralyand Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.Ralyand.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: ''}));
        }
    ...
    bestPing.remarks = '😶‍🌫️ Ralyand Frag - Best Ping 💥';
    ...
    bestFragment.remarks = '😶‍🌫️ Ralyand Frag - Best Fragment 😎';
...
const getWarpConfigs = async (env, client) => {
    ...
    xrayWarpBestPing.remarks = '😶‍🌫️ Ralyand - Warp Best Ping 🚀'
    ...
    xrayWarpOutbounds.forEach((outbound, index) => {
        xrayWarpConfigs.push({
            ...xrayWarpConfig,
            remarks: `😶‍🌫️ Ralyand - Warp ${index + 1} 🇮🇷`,
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `😶‍🌫️ Ralyand - WoW ${index/2 + 1} 🌍`;
...
const buildWarpOutbounds = async (env, client, remoteDNS, localDNS, blockAds, bypassIran, blockPorn, bypassLAN, warpEndpoints) => {
    ...
    try {
        warpConfigs = await env.Ralyand.get('warpConfigs', {type: 'json'});
        proxySettings = await env.Ralyand.get('proxySettings', {type: 'json'});
    } catch (error) {
        console.log(error);
...
const fetchWgConfig = async (env, warpKeys) => {
    ...
    await env.Ralyand.put('warpConfigs', JSON.stringify(warpConfigs));
}
...
const updateDataset = async (env, Settings) => {
    ...
    try {
        currentProxySettings = await env.Ralyand.get("proxySettings", {type: 'json'});
    } catch (error) {
        console.log(error);
    ...
    try {    
        await env.Ralyand.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
...
const Authenticate = async (request, env) => {
    ...
    try {
        const secretKey = await env.Ralyand.get('secretKey');
...
const renderHomePage = async (env, hostName, fragConfigs) => {
    ...
    try {
        proxySettings = await env.Ralyand.get('proxySettings', {type: 'json'});
        warpConfigs = await env.Ralyand.get('warpConfigs', {type: 'json'});
        password = await env.Ralyand.get('pwd');
    } catch (error) {
        console.log(error);
	...
	<head>
        ...
        <title>Ralyand Panel ${panelVersion}</title>
	...
	<body>
		<h1>Ralyand Panel <span style="font-size: smaller;">${panelVersion}</span> 😶‍🌫️</h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#Ralyand-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#Ralyand-Normal', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#Ralyand-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#Ralyand-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#Ralyand-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}#Ralyand Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}#Ralyand Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}#Ralyand-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}#Ralyand-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=singbox#Ralyand-Warp', 'Warp Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=singbox#Ralyand-Warp', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
			...
			<div class="table-container">
				<table id="warp-pro-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/warpsub/${userID}?app=nikang#Ralyand-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=nikang#Ralyand-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
					</tr>
					<tr>
						...
						<td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/warpsub/${userID}?app=hiddify#Ralyand-Warp-Pro', 'Warp Pro Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/warpsub/${userID}?app=hiddify#Ralyand-Warp-Pro', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/Ralyand-Worker-Panel" target="_blank">Github</a>
    ...
    <body>
        <div id="error-container">
            ...
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/bia-pain-bache/Ralyand-Worker-Panel/blob/main/README.md">documents</a>' 
                    : ''}
                </h2>
